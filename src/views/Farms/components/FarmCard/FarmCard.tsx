...
import { Flex, Text, Skeleton } from '@zealous4467/cmbstswapuikit'
...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice, bnbPrice, ethPrice, ethereum, account }) => {
  ...
  const totalValue: BigNumber = useMemo(() => {
    ...
    if (farm.quoteTokenSymbol === QuoteToken.COMBUST) {
  ...
  const earnLabel = farm.dual ? farm.dual.earnLabel : 'COMBUST'
  ...
  return (
    <FCard>
      {farm.tokenSymbol === 'COMBUST' && <StyledCardAccent />}
      <CardHeading
        lpLabel={lpLabel}
